...
m.2wk.agg <- aggregate(abs_err_2wk_perc ~ leadpkwk_mean + model + oev_base, data = m.temp.2wk, FUN = mean)
m.2wk.agg$metric <- '2 Week'
m.3wk.agg <- aggregate(abs_err_3wk_perc ~ leadpkwk_mean + model + oev_base, data = m.temp.3wk, FUN = mean)
m.3wk.agg$metric <- '3 Week'
m.4wk.agg <- aggregate(abs_err_4wk_perc ~ leadpkwk_mean + model + oev_base, data = m.temp.4wk, FUN = mean)
...
m.1wk.agg$metric <- '1 Week'
m.2wk.agg <- aggregate(abs_err_2wk_perc ~ FWeek_pkwk + model + oev_base, data = m.temp.2wk, FUN = mean)
m.2wk.agg$metric <- '2 Week'
m.3wk.agg <- aggregate(abs_err_3wk_perc ~ FWeek_pkwk + model + oev_base, data = m.temp.3wk, FUN = mean)
m.3wk.agg$metric <- '3 Week'
m.4wk.agg <- aggregate(abs_err_4wk_perc ~ FWeek_pkwk + model + oev_base, data = m.temp.4wk, FUN = mean)
m.4wk.agg$metric <- '4 Week'
names(m.1wk.agg)[4] = names(m.2wk.agg)[4] = names(m.3wk.agg)[4] = names(m.4wk.agg)[4] = 'mae'
m.agg <- rbind(m.1wk.agg, m.2wk.agg, m.3wk.agg, m.4wk.agg)
m.agg$metric <- factor(m.agg$metric)
