...
### Kernel Density ###
# Note: For PI and 1-4 weeks only?
bin.sizes <- c(250, 500)
e.all = e.wk.all = NULL
...
for (bin.size in bin.sizes) {
  ...
  ### Peak Intensity ###
  # Read in results:
  e <- read.csv(list.files(pattern = 'Ens_PI'))
  ...
  levels(m$obs_peak_int_bin) <- c(seq(bin.size, 14000, by = bin.size), 100000)
  m$obs_peak_int_bin <- as.numeric(as.character(m$obs_peak_int_bin))
  
  # Merge:
  e <- merge(e, m, by = c('country', 'season'))
  
  # Find % of normal distribution within observed bin:
  e$lower <- e$obs_peak_int_bin - bin.size; e$upper <- e$obs_peak_int_bin
  ...
  ### 1-4 Weeks ###
  # Read in results:
  e1 <- read.csv(list.files(pattern = 'Ens_1wk'))
  e2 <- read.csv(list.files(pattern = 'Ens_2wk'))
  e3 <- read.csv(list.files(pattern = 'Ens_3wk'))
  e4 <- read.csv(list.files(pattern = 'Ens_4wk'))
  ...
  levels(m$obs_xweek_bin) <- c(seq(bin.size, 14000, by = bin.size), 100000)
  m$obs_xweek_bin[m$obs_xweek == 0 & !is.na(m$obs_xweek)] <- bin.size
  ...
  e$lower <- e$obs_xweek_bin - bin.size; e$upper <- e$obs_xweek_bin
  probs <- sapply(1:dim(e)[1], function(ix) {
    pnorm(e[ix, 'upper'], mean = e[ix, 'mean'], sd = e[ix, 'sd']) - pnorm(e[ix, 'lower'], mean = e[ix, 'mean'], sd = e[ix, 'sd'])
  })
  e$prob <- probs
  ...
  e <- merge(e, m, by = c('season', 'country', 'run', 'fc_start', 'oev_base', 'oev_denom', 'lambda'))
  e$metric2 <- bin.size
  e <- e[, c(1:8, 16, 9:10, 12:15, 11)]
  
  # Store:
  e.wk.all <- rbind(e.wk.all, e)
  rm(e, m, means, probs, sds)
  
}

# Save PI and 1-4 wks:
write.csv(e.all, file = 'logScores_pi_kd.csv', row.names = FALSE)
write.csv(e.wk.all, file = 'logScores_1-4wks_kd.csv', row.names = FALSE)

# Clean up:
rm(bin.size, bin.sizes, e.all, e.wk.all)
