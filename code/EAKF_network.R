...
EAKF_rFC <- function(num_ens, tmstep, param.bound, obs_i = obs_i, ntrn = 1, obs_vars,
  ...
  for (tt in 1:ntrn) {
    ...
    if (tt >= 5) {
      ...
      for (i in (1:n)[!is.na(obs_i[tt, ])]) {
        ...
        lines((tt + 1):(tt + nfc), obs.fcast.toPlot.ind, type = 'b', pch = 20, lty = 1, cex = 0.8, col = 'coral')
      }
      
      # Also calculate S and I by country
      ...
      statesfcast <- cbind(fc_start, 'fcast', rep(tstep[(tt + 1):nsn], n), melt(sfcast_mean), melt(sfcast_sd), melt(ifcast_mean), melt(ifcast_sd), melt(obsfcast_mean), melt(obsfcast_sd));
      statesfcast <- statesfcast[, c(1:6, 9, 12, 15, 18, 21)]; names(statesfcast) <- c('fc_start', 'result', 'time', 'week', 'country', 'S', 'S_sd', 'I', 'I_sd', 'newI', 'newI_sd');
      statesfcast$week <- statesfcast$week + wk_start + tt - 1; statesfcast$country <- countries[statesfcast$country]
      
      # Rbind to fc.op!
      fc.op <- rbind(fc.op, statesfcast)
      
      # Get temporary xpost
      obspost.temp <- obspost[,, 1:tt]
      obspost_mean.temp <- t(apply(obspost.temp, c(1, 3), mean))
      
      # Evaluate fcast for 'metrics' frame
      Y <- rbind(obspost_mean.temp, obsfcast_mean)
      Y.ens <- array(NA, c(n, num_ens, nsn))
      Y.ens[,, 1:tt] <- obspost.temp; Y.ens[,, (tt + 1):(tt + nfc)] <- obsfcast
      ...
      for (i in 1:n) {
        ...
        if (!all(is.na(obs_i[, i]))) {
          ...
          corr_fcast[i] <- cor(obsfcast_mean[, i], obs_i[(tt + 1):nsn, i], use = 'pairwise.complete.obs')
          rmse_fcast[i] <- sqrt(mean((obsfcast_mean[, i] - obs_i[(tt + 1):nsn, i]) ** 2, na.rm = TRUE))
          
          obs_adj <- obs_i[(tt + 1):nsn, i]
          obs_adj[obs_adj == 0 & !is.na(obs_adj)] <- 1.0
          mape[i] <- sum(abs((obs_adj - obsfcast_mean[, i]) / obs_adj), na.rm = TRUE) * 100 / nfc
          wape[i] <- sum(abs(obs_i[(tt + 1):nsn, i] - obsfcast_mean[, i]), na.rm = TRUE) / sum(obs_i[(tt + 1):nsn, i], na.rm = TRUE) * 100 - nfc
          smape[i] <- sum(abs(obsfcast_mean[, i] - obs_i[(tt + 1):nsn, i]) / ((obsfcast_mean[, i] + obs_i[(tt + 1):nsn, i]) / 2), na.rm = TRUE) * 100 / nfc
          
          # 1-4 weeks ahead:
          if (dim(obsfcast_mean)[2] > 0 & !is.na(obs_i[tt + 1, i])) {
            obs1wk[i] <- obs_i[tt + 1, i]
            fcast1wk[i] <- obsfcast_mean[1, i]
            deno_obs_i <- ifelse(obs_i[tt + 1, i] == 0, 1.0, obs_i[tt + 1, i])
            rdiff_next_newI[i] <- (fcast1wk[i] - deno_obs_i) / deno_obs_i
          }
          
          if (dim(obsfcast_mean)[2] > 1 & !is.na(obs_i[tt + 2, i])) {
            obs2wk[i] <- obs_i[tt + 2, i]
            fcast2wk[i] <- obsfcast_mean[2, i]
            deno_obs_i <- ifelse(obs_i[tt + 2, i] == 0, 1.0, obs_i[tt + 2, i])
            rdiff_next_newI2[i] <- (fcast2wk[i] - deno_obs_i) / deno_obs_i
          }
          
          if (dim(obsfcast_mean)[2] > 2 & !is.na(obs_i[tt + 3, i])) {
            obs3wk[i] <- obs_i[tt + 3, i]
            fcast3wk[i] <- obsfcast_mean[3, i]
            deno_obs_i <- ifelse(obs_i[tt + 3, i] == 0, 1.0, obs_i[tt + 3, i])
            rdiff_next_newI3[i] <- (fcast3wk[i] - deno_obs_i) / deno_obs_i
          }
          
          if (dim(obsfcast_mean)[2] > 3 & !is.na(obs_i[tt + 4, i])) {
            obs4wk[i] <- obs_i[tt + 4, i]
            fcast4wk[i] <- obsfcast_mean[4, i]
            deno_obs_i <- ifelse(obs_i[tt + 4, i] == 0, 1.0, obs_i[tt + 4, i])
      ...
      # Rbind mets, dists, and ens to main outputs
      fc.met <- rbind(fc.met, cbind(fc_start, countries, obs_pkwk + wk_start - 1, pkwk_mode + wk_start - 1, delta_pkwk_mode, pkwk_mean + wk_start - 1, delta_pkwk_mean,
      ...
      tmp.dist <- rbind(cbind('onset3', onsets3Dist), cbind('onset4', onsets4Dist), cbind('onset5', onsets5Dist), cbind('onset6', onsets6Dist),
                        cbind('pw', peakWeeksDist), cbind('pi', peakIntensitiesDist))
      for (i in 1:4) {
        tmp.dist <- rbind(tmp.dist, cbind(paste0('nextweek', i), nextILIDist[,, i]))
      }
      tmp.dist <- as.data.frame(cbind(fc_start, tmp.dist))
      names(tmp.dist) <- c('fc_start', 'metric', 'bin', countries)
      tmp.dist <- melt(tmp.dist, id.vars = c('fc_start', 'metric', 'bin'))
      names(tmp.dist)[4] <- 'country'
      fc.dist <- rbind(fc.dist, tmp.dist)
      
      tmp.ens <- rbind(cbind(fc_start, countries, 'pi', peakIntensities), cbind(fc_start, countries, 'ar', totalARs))
      for (i in 1:4) {
        tmp.ens <- rbind(tmp.ens, cbind(fc_start, countries, paste0(i, 'week'), nextILI[,, i]))
      }
      tmp.ens <- as.data.frame(tmp.ens)
      names(tmp.ens) <- c('fc_start', 'country', 'metric', 1:300)
      fc.ens <- rbind(fc.ens, tmp.ens)
  ...
  out2 <- fc.op[, c(1, 3:4, 2, 5:6, 8, 10, 7, 9, 11)]
  out3 <- fc.met
