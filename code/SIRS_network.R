...
propagateToySIRS <- function(tm_strt, tm_end, tm_step, S0, I0, N, D, L, beta, realdata = FALSE) {
  cnt <- 1
  ...
  for (t in tm_vec) {
    ...
    # # Artifically inflate air travel?
    # a.temp <- a.temp * 0
    
    # # Artificially inflate (or remove) train data:
    # t.rand <- t.rand * 0
    
    # Add "random" travel data:
    all.rand <- a.temp + t.rand
