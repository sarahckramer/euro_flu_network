# EAKF mainCode for forecasting and fitting network model to observations #

import pandas as pd
from run_models.SIRS_python import *
from run_models.functions_all import *


# EAKF mainCode for network model (forecasting)
    ...
    for tt in range(ntrn):
        ...
        for loc in range(n):
            ...
            if not np.isnan(obs_i[tt, loc]) and not np.isnan(obs_var) and obs_var > 0:
                ...
                # Question: Do we want to check for S0 values being reduced below 0, like in R mainCode?
        ...
        if tt >= 4:
            ...
            # this will give the actual week value, not a python_init index
...
# EAKF mainCode for network model (fitting only)
# noinspection PyShadowingNames
