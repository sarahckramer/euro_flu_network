...
fits.df.full <- fits.df.full[, c(1:6, 14:17)]
...
fits.df.plot <- melt(fits.df.plot, id.vars = c('country', 'season', 'subtype', 'run', 'region', 'scaling.range', 'data.type', 'onsetObs5'))
names(fits.df.plot)[9] <- 'param'
...
ggsave(filename = 'results/plots/FigS14.svg', plot = p1, width = 12, height = 8)
