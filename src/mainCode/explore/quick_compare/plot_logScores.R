...
if (byWeek == 'Predicted') {
  ...
  d.agg <- aggregate(score ~ leadpkwk_mean + oev_base + oev_denom + model, data = d.temp, FUN = mean)
  ...
  p3 <- ggplot(data = d.agg) + geom_line(aes(x = leadpkwk_mean, y = score, col = model)) +
    ...
    labs(x = 'Predicted Lead Week', y = 'Mean Log Score') + scale_x_continuous(breaks = -8:4)
  ...
  d.agg <- aggregate(score ~ leadonset5 + oev_base + oev_denom + model, data = d.temp, FUN = mean)
  d.agg$group <- paste(d.agg$leadpkwk_mean, d.agg$model, d.agg$oev_base, d.agg$oev_denom, sep = '_')
  ...
  e.agg <- aggregate(score ~ leadpkwk_mean + oev_base + oev_denom + model, data = e.temp, FUN = mean)
  ...
  # d.agg <- aggregate(score ~ leadpkwk_mean + model, data = d.temp, FUN = mean)
  ...
  # e.agg <- aggregate(score ~ leadpkwk_mean + model, data = e.temp, FUN = mean)
  ...
  # d.agg <- aggregate(score ~ leadonset5 + model, data = d.temp, FUN = mean)
  ...
  #   labs(x = 'Predicted Lead Week', y = 'Mean Log Score') + scale_x_continuous(breaks = -8:4)
  # 
  # pdf('mainCode/gridSearch/plots/pres_comp_logScores_byPred.pdf', width = 8, height = 9)
  # grid.arrange(p1, p2, p3)
  # dev.off()
  ...
  d.agg <- aggregate(score ~ FWeek_pkwk + oev_base + oev_denom + model, data = d.temp, FUN = mean)
  ...
  p3 <- ggplot(data = d.agg) + geom_line(aes(x = FWeek_pkwk, y = score, col = model)) +
    ...
    theme_bw() + scale_color_brewer(palette = 'Set1') +
    labs(x = 'Observed Lead Week', y = 'Mean Log Score') + scale_x_continuous(breaks = -8:4)
  grid.arrange(p1, p2, p3)
  ...
  # d.agg <- aggregate(score ~ FWeek_onwk + oev_base + oev_denom + model, data = d.temp, FUN = mean)
  ...
  e.agg <- aggregate(score ~ FWeek_pkwk + oev_base + oev_denom + model, data = e.temp, FUN = mean)
...
} else if (byWeek == 'Observed_All') {
  # Start with PT:
  
  if (restrict.fc) {
    d.temp <- d[d$FWeek_pkwk >= -8 & d$FWeek_pkwk < 5 & d$metric == 'pt' & d$fc_start >= 50 & d$fc_start < 65, ]
  } else {
    d.temp <- d[d$FWeek_pkwk >= -8 & d$FWeek_pkwk < 5 & d$metric == 'pt', ]
  }
  ...
  # OT:
  if (restrict.fc) {
    d.temp <- d[d$FWeek_onwk >= -6 & d$FWeek_onwk < 7 & d$metric == 'ot' & d$fc_start < 65, ]
  } else {
  ...
  # PI:
  if (restrict.fc) {
    e.temp <- e.pi[e.pi$FWeek_pkwk >= -8 & e.pi$FWeek_pkwk < 5 & e.pi$fc_start >= 50 & e.pi$fc_start < 65, ]
  } else {
    e.temp <- e.pi[e.pi$FWeek_pkwk >= -8 & e.pi$FWeek_pkwk < 5, ]
  }
...
rm(d.agg, d.temp, e.agg, e.temp, p1, p2, p3)
if (exists('wk')) {
  rm(wk)
}
