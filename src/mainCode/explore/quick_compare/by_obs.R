### Plot simple accuracy metric by observed lead week ###

# Plot accuracy by observed lead by model/oev_base/oev_denom:
...
for (o1 in levels(m$oev_base)) {
  for (o2 in levels(m$oev_denom)) {
    for (model.type in levels(m$model)) {
      m.temp <- m.temp1[m.temp1$oev_base == o1 & m.temp1$oev_denom == o2  & m.temp1$model == model.type, ]
      m.temp.ot <- m.temp2[m.temp2$oev_base == o1 & m.temp2$oev_denom == o2  & m.temp2$model == model.type,]
      ...
      if (dim(m.temp)[1] > 0) {
        ...
        res$oev_base <- o1; res$oev_denom <- o2; res$model <- model.type
...
res.all$group <- paste(res.all$metric, res.all$oev_base, res.all$oev_denom, res.all$model, sep = '_'); res.all$group <- factor(res.all$group)

p1 <- ggplot(data = res.all[res.all$metric == 'pt', ], aes(x = lead, y = value, group = group, col = model)) + geom_line() + 
  geom_point(aes(size = len)) + scale_color_brewer(palette = 'Set1') + facet_wrap(~oev_denom) +
...
# p1 <- ggplot(data = res.all[res.all$metric == 'pt', ], aes(x = lead, y = value, group = group, col = oev_base, pch = oev_denom)) + geom_line() + 
#   geom_point(aes(size = len)) + scale_color_brewer(palette = 'Set1') +
#   theme_bw() + labs(x = 'Observed Lead Week', y = '% Accurate (within 1 week)', title = 'Peak Timing Accuracy') +
#   coord_cartesian(ylim = c(0,1)) + scale_x_continuous(breaks = unique(res.all$lead), labels = unique(res.all$lead))
# p2 <- ggplot(data = res.all[res.all$metric == 'pi', ], aes(x = lead, y = value, group = group, col = oev_base, pch = oev_denom)) + geom_line() + 
#   geom_point(aes(size = len)) + scale_color_brewer(palette = 'Set1') +
#   theme_bw() + labs(x = 'Observed Lead Week', y = '% Accurate (within 25%)', title = 'Peak Intensity Accuracy') +
#   coord_cartesian(ylim = c(0,1)) + scale_x_continuous(breaks = unique(res.all$lead), labels = unique(res.all$lead))
# p3 <- ggplot(data = res.all[res.all$metric == 'ot', ], aes(x = lead, y = value, group = group, col = oev_base, pch = oev_denom)) + geom_line() + 
#   geom_point(aes(size = len)) + scale_color_brewer(palette = 'Set1') +
#   theme_bw() + labs(x = 'Observed Lead Week', y = '% Accurate (within 1 week)', title = 'Onset Timing Accuracy') +
#   coord_cartesian(ylim = c(0,1)) + scale_x_continuous(breaks = unique(res.all$lead), labels = unique(res.all$lead))

plots.by.obs <- list(p1, p2, p3)
rm(m.temp, m.temp.ot, m.temp1, m.temp2, res, res.all, res.ot, res.pt, res.pi, o1, o2, model.type, p1, p2, p3)
