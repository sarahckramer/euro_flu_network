...
buildOutputNew<-function(res_sub, format='long'){
  required_order=NULL
  if(format == 'long'){
  ...
  for(i in 2:length(required_order)){
    if(required_order[i] %in% colnames(res_sub)){
      ...
      if (i == 2) {
        out = cbind(out, res_sub[,required_order[i]])
      } else {
        out = cbind(out, round(as.numeric(res_sub[,required_order[i]]), digits=d))
      }
...
convertDaysToDate<-function(res, start_date){
  ...
  if(length(time_index) != 1){#Could not find the index, return result as is
    ...
    for(id in 1:nrow(res))
      time_column = append(time_column, as.Date(as.numeric(res[id, time_index])-1, origin=paste(1900+as.POSIXlt(start_date)$year,"01","01",sep="-")));
