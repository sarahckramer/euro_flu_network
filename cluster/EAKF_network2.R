...
EAKF_rFC <- function(num_ens, tmstep, param.bound, obs_i = obs_i, ntrn = 1, obs_vars,
  ...
  # So <- matrix(0, n ** 2 * 2 + 6, num_ens) # last 5 rows are parameters
  xprior <- array(0, c(n ** 2 * 3 + 6, num_ens, ntrn + 1)) # add newI for each compartment, too
  xpost <- array(0, c(n ** 2 * 3 + 6, num_ens, ntrn))
  ...
  param.indices <- (max(newI.indices) + 1):(max(newI.indices) + 6) # where the epi parameters are stored
  ...
  parms <- parms[(dim(parms)[1] - 5):(dim(parms)[1]), ] # L, D, R0mx, R0mn, airScale, nu
  ...
  D.temp <- parms[2, ]; L.temp <- parms[1, ]; airScale.temp <- parms[5, ]; nu.temp <- parms[6, ]
  ...
  Sr_tmp <- sapply(1:num_ens, function(ix) {
    propagateToySIRS(tm_strt = tcurrent + dt, tm_end = tcurrent + tmstep, dt,
                     ...
                     airScale = airScale.temp[ix], nu = nu.temp[ix],
                     realdata = TRUE)
  })
  ...
  for (tt in 1:ntrn) {
    ...
    inflat <- diag(x = rep(lambda, n ** 2 * 3 + 6), n ** 2 * 3 + 6, n ** 2 * 3 + 6)
    ...
    airScale.temp <- xpost[param.indices[5],, tt]; nu.temp <- xpost[param.indices[6],, tt]
  ...
  airScale.temp <- xpost[param.indices[5],, ntrn]; nu.temp <- xpost[param.indices[6],, tt]
  ...
  names(params.post_df) <- c('L', 'D', 'R0mx', 'R0mn', 'airScale', 'nu', 'L_sd', 'D_sd', 'R0mx_sd', 'R0mn_sd', 'airScale_sd', 'nu_sd')
